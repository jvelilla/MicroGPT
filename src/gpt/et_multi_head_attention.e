...
feature -- Operation
    ...
    forward (x: ET_TENSOR [ET_NUMERIC_ELEMENT [REAL_32]]): ET_TENSOR [ET_NUMERIC_ELEMENT [REAL_32]]
        ...
        local
            ...
            qkv, q, k, v: ET_TENSOR [ET_NUMERIC_ELEMENT [REAL_32]]
            q_view, k_view, v_view: ET_TENSOR [ET_NUMERIC_ELEMENT [REAL_32]]
            q_h, k_h, v_h: ET_TENSOR [ET_NUMERIC_ELEMENT [REAL_32]]
            att_scores, att_probs, att_exp, att_sum: ET_TENSOR [ET_NUMERIC_ELEMENT [REAL_32]]
            out_h, out_h_t, out_flat: ET_TENSOR [ET_NUMERIC_ELEMENT [REAL_32]]
            scale: REAL_64
            ...
            k_h_t: ET_TENSOR [ET_NUMERIC_ELEMENT [REAL_32]]
            causal_mask: ET_TENSOR [ET_NUMERIC_ELEMENT [REAL_32]]
            t1, t2: INTEGER
            l_elem: ET_NUMERIC_ELEMENT [REAL_32]
            k_full, v_full: ET_TENSOR [ET_NUMERIC_ELEMENT [REAL_32]]
        do
            ...
            -- 1. Project
            qkv := c_attn.forward (x)
            
            -- 2. Slice Q, K, V
            q := qkv.slice_range (qkv.shape.count, 1, C)
            k := qkv.slice_range (qkv.shape.count, C + 1, 2 * C)
            v := qkv.slice_range (qkv.shape.count, 2 * C + 1, 3 * C)
            ...
            att_sum := att_exp.sum (att_exp.shape.count, False)
            att_probs := att_exp / att_sum.unsqueeze (att_sum.shape.count + 1)
            ...
            -- Final Projection
            Result := c_proj.forward (out_flat)
