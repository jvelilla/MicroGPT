...
feature -- Operation
    ...
    forward (idx: ARRAY [INTEGER]; targets: detachable ARRAY [INTEGER]): TUPLE [logits: LIST [LIST [ET_VALUE]]; loss: detachable ET_VALUE]
        ...
        local
            t: INTEGER
            tok_emb: LIST [LIST [ET_VALUE]]
            pos_emb: LIST [LIST [ET_VALUE]]
            x: LINKED_LIST [LIST [ET_VALUE]] -- (T, C)
            i, j: INTEGER
            row_t, row_p: ARRAYED_LIST [ET_VALUE]
            row_sum: LINKED_LIST [ET_VALUE]
            
            curr_x: LIST [LIST [ET_VALUE]]
            

            final_norm_x: LIST [LIST [ET_VALUE]]
            logits_out: LIST [LIST [ET_VALUE]]
            
            loss_val: ET_VALUE
            target_t: INTEGER
            logits_t: LIST [ET_VALUE]
            reshaped_logits: ARRAYED_LIST [ET_VALUE]
            sum_val, log_sum: ET_VALUE
            
            logits_arr: ARRAYED_LIST [LIST [ET_VALUE]]
        do
            t := idx.count
            -- require valid_len: t <= max_seq_len
            
            -- 1. Embeddings
            create {LINKED_LIST [LIST [ET_VALUE]]} tok_emb.make
            across idx as id loop
                tok_emb.extend (token_embedding_table.forward (id))
            end
            
            create {LINKED_LIST [LIST [ET_VALUE]]} pos_emb.make
            ...
            -- 3. Final Layer Norm
            create {LINKED_LIST [LIST [ET_VALUE]]} final_norm_x.make
            across curr_x as row_x loop
                final_norm_x.extend (ln_f.forward (row_x))
            end
            
            -- 4. LM Head (logits)
            create {LINKED_LIST [LIST [ET_VALUE]]} logits_out.make
            ...
            if attached targets as tgt then
                loss_val := create {ET_VALUE}.make (0.0)
                ...
                from i := 1 until i > t loop
                   ...
                   -- log(sum(exp(x)))
                   sum_val := create {ET_VALUE}.make (0.0)
                ...
                loss_val := loss_val * create {ET_VALUE}.make (1.0 / t)
    ...
    generate (idx: ARRAY [INTEGER]; max_new_tokens: INTEGER; temperature: REAL_64; stop_token: INTEGER): ARRAYED_LIST [INTEGER]
        ...
        local
            ...
            i_step: INTEGER
            output_tuple: TUPLE [logits: LIST [LIST [ET_VALUE]]; loss: detachable ET_VALUE]
            next_token: INTEGER
            cond_idx: ARRAY [INTEGER]
        do
            create curr_idx.make_from_array (idx)
            
            -- Initialize Cache with sufficient capacity
            init_cache (idx.count + max_new_tokens + 5)
            
            -- 1. Prefill (Forward full prompt)
            output_tuple := forward (curr_idx.to_array, Void)
            
            -- Get last logits and sample
            next_token := sample_token (output_tuple.logits.last, temperature)
            
            if next_token /= stop_token then
                curr_idx.extend (next_token)
                
                -- 2. Generation Loop (Forward one token at a time)
                from i_step := 2 until i_step > max_new_tokens loop
                    
                    -- Cond is just the last token
                    output_tuple := forward (<<next_token>>, Void)
    ...
    sample_token (logits: LIST [ET_VALUE]; temperature: REAL_64): INTEGER
        local
            vocab_probs: ARRAYED_LIST [REAL_64]
            last_logits_arr: ARRAYED_LIST [ET_VALUE]
            sum_exp: REAL_64
            k: INTEGER
            logit_val: REAL_64
            r, acc: REAL_64
        do
            create last_logits_arr.make_from_iterable (logits)
            create vocab_probs.make (logits.count)
            sum_exp := 0.0
            
            from k := 1 until k > logits.count loop
