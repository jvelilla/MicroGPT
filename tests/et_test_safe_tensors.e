...
feature -- Tests
    ...
    test_safetensors_loading
        local
            ...
            t: detachable ET_TENSOR [REAL_32]
            f: RAW_FILE
            header_json: STRING
        ...
        do
            ...
            if attached t as tensor then
                ...
                assert ("Value at 1,1 is 1.0", (tensor.item (<<1, 1>>) - 1.0).abs < 1.0e-6)
                assert ("Value at 1,2 is 2.0", (tensor.item (<<1, 2>>) - 2.0).abs < 1.0e-6)
                assert ("Value at 2,1 is 3.0", (tensor.item (<<2, 1>>) - 3.0).abs < 1.0e-6)
                assert ("Value at 2,2 is 4.0", (tensor.item (<<2, 2>>) - 4.0).abs < 1.0e-6)
            end
